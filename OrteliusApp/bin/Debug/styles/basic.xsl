...
<script language="JavaScript" type="text/javascript" src="basic/script.js"></script>
<link rel="stylesheet" href="basic/style.css" type="text/css" media="screen"/>
</head>
<body>
...
<div class="orteliusRemark">Created with Ortelius - <a href="http://ortelius.marten.dk" target="_blank">ortelius.marten.dk</a></div>
...
<xsl:value-of select="$className"/><br />
</xsl:if>
<br/>
...
<xsl:if test="extends">
<div class="elementTitle">Extends/inherit:</div>
...
<img src="basic/arrowright.gif" border="0" height="8" width="18"/> <a>
...
<xsl:for-each select=".">
<xsl:comment>Details of each method</xsl:comment>
<xsl:variable name="className" select="name"/>
...
<div class="backButton"><img src="basic/arrowleft.gif" border="0" height="8" width="18"/> <a onmousedown="goBack()">Back</a></div>
...
<xsl:comment>Details of each event</xsl:comment>
<xsl:for-each select="event">
<xsl:variable name="filename"><xsl:value-of select="$basePath" />/ortfiles/<xsl:value-of select="$packageName" /><xsl:value-of select="$className" />.<xsl:value-of select="name" />.html</xsl:variable>
...
<div class="treeBranch"><xsl:value-of disable-output-escaping="yes" select="@name"/></div>
...
<img src="basic/arrowright.gif" border="0" height="8" width="18"/> 
